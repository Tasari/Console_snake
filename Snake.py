...
class Snake:
    ...
    def move(self, direction, score, speed, drunk_fruit_flag, boring_fruit_flag, drunk_counter, boring_counter):
        if direction == 'w':
            ...
            return score, speed, drunk_fruit_flag, boring_fruit_flag, drunk_counter, boring_counter
        self.moves.append(direction)
        if move in impassable_symbols.values():
            game_over(score)
        elif move in fruits.values():
            score, speed, drunk_fruit_flag, boring_fruit_flag, drunk_counter, boring_counter = trigger(move, self.map, score, speed,drunk_fruit_flag, boring_fruit_flag, drunk_counter, boring_counter)
            self.map.spawn_fruit(val2key(fruits, move))
        else:
            self.delete()  
        if direction == 'w':
            self.map.board[self.move_x][self.move_y-1] = impassable_symbols['snake_symbol']
            self.move_y -= 1
        elif direction == 'a':
            self.map.board[self.move_x-1][self.move_y] = impassable_symbols['snake_symbol']
            self.move_x -= 1
        elif direction == 's':
            self.map.board[self.move_x][self.move_y+1] = impassable_symbols['snake_symbol']
            self.move_y += 1
